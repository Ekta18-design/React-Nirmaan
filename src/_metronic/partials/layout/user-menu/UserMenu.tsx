...
// Helper to clear userid from both localStorage and cookies
const clearUserId = () => {
...
const UserMenu: React.FC<UserMenuProps> = ({ user })=> {
  ...
  const handleLogout = () => {
    // Clear the userid from localStorage and cookies
    clearUserId();
    navigate('auth/login');
  };
  return (
    <div className='menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg menu-state-primary fw-bold py-4 fs-6 w-275px' data-kt-menu='true' >
        ...
        <div className='menu-content d-flex align-items-center px-3'>
          ...
          <div className='d-flex flex-column'>
            <div className='fw-bolder d-flex align-items-center fs-5'>
              {user?.user_firstname || 'First Name'} {user?.user_lastname || 'Last Name'}
      ...
      <div className='menu-item px-5'>
        <Link to={'/crafted/pages/profile'} className='menu-link px-5'>My Profile</Link>
       </div>
      {/* end::Menu item */}
            ...
            <div className='menu-content px-3'>
              <label className='form-check form-switch form-check-custom form-check-solid'>
                <input className='form-check-input w-30px h-20px' type='checkbox' value='1'
                  checked={true} name='notifications'onChange={() => {}}/>
      ...
      {/* begin::Menu item */}
      <div className='menu-item px-5'>
        <a onClick={handleLogout} className='menu-link px-5'> Sign Out </a>
      </div>
    </div>
  )
}
